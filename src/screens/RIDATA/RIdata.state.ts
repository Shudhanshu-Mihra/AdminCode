...
import { setRecieptInvoiceData } from './reducer/RIdata.reducer';
...
export const useRIdataState = () => {
  ...
  // const recieptInvoiceData = useSelector(
  //   (state: IState) => state.RIdata.recieptInvoiceData
  // );
  const { 
    user: {
      userInfo: { company },
  ...
  const onFetchReceiptsHandler = useCallback(async (params?: IGetReceiptsInvoiceParams) => {
    try {
      ...
      // Assuming the API response has the data array
        dispatch(setRecieptInvoiceData(data.data));
        
      setState(prevState => ({
        ...prevState,
        recieptInvoiceData: data.data || [],
        isEmptyData: data?.length === 0,
        isFetchingReceipts: false,
          isContentLoading: false,
      }));
        for (let i = 0; i < recieptInvoiceData.length;i++){
            
            console.log(state.recieptInvoiceData[i]);
        }
    } catch (error) {
      setState(prevState => ({
        ...prevState,
